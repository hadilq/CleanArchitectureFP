...
import com.github.hadilq.cleanarchitecturefp.util.ViewActionUtil.waitIndex
...
class ArtistsActivityTest {
    ...
    fun launchActivityCheckList() {
        ...
        `when`(api.searchArtists(any())).thenAnswer { Single.just(dto) }
        ...
        onView(isRoot())
            .perform(
                waitIndex(
                    withId(R.id.nameView), 0,
                    TimeUnit.SECONDS.toMillis(15),
                    matches(withText("Eminem"))
                )
            )
    }
